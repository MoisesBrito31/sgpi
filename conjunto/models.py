...
class Componente(Base):
    ...
    preco_venda = models.DecimalField('Preço Venda', decimal_places=2, max_digits=8, blank=True)
...
class Conjunto(Base):
    ...
    valor = models.DecimalField('Preço', max_digits=8, decimal_places=2,blank=True)
...
class Relacao(Base):
    ...
    def __str__(self):
        return f'{self.componente.nome} x {quantidade}'

"""
@receiver(pre_save, sender=Conjunto)
def calcular(sender,instance,*args,**kargs):
    try:
        qtd = instance.relacao.quantidade
        preco = instance.relacao.componente.preco_venda
    except:
        qtd = 0.00
        preco = 0.00
    instance.valor = qtd * preco
