<template>
    <div>
        <div class="card-header mb-3">
            <h4 v-if="industriaID!==undefined">Edição de Indústria</h4>
            <h4 v-else>Cadastro de Indústria</h4>
        </div>
        <b-form>
...
export default {
    ...
    methods:{
        ...
        async Put(valor){ 
            ...
            this.espera=true
            fetch(`${this.dominio}/api/clientes-industria/${pk}`,{
                method:met,
                headers:{
                    'Authorization': `Token ${window.localStorage.getItem('api-token')}`                     
                },
        ...
        load(){
            console.log(this.industriaID)
            if (this.industriaID!==undefined){
                this.dataLoad(this.industriaID)
            }
        }
    },
    props:{
