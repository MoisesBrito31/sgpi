<template>
    <div>
        <div class="card-header mb-3">
            <h4 v-if="linhaID!==undefined">Edição de Linha</h4>
            <h4 v-else>Cadastro de Linha</h4>
        </div>
        <b-form>
        <b-form-group label="Linha:" :state="nomeOK"  invalid-feedback="Nome Esperado">
            <b-input-group>
                <b-form-input :state="nomeOK" 
                    type="text" placeholder="Linha" v-model="obj.nome" trim >
                </b-form-input>
            </b-input-group>
        </b-form-group>
...
export default {
    ...
    methods:{
        ...
        load(){
            if (this.linhaID!==undefined){
                this.dataLoad(this.linhaID)
            }
        }
    },
    props:{
